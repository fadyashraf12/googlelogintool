...
# --- The rest of the functions (human_type, human_click, etc.) remain the same ---
...
# ... (all other helper functions like auto_skip, is_logged_in, etc. are unchanged)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if log_callback:
                ...
                log_callback(f"Please ensure Google Chrome is running and was launched with the '--remote-debugging-port=9222' flag.")
                log_callback("Right-click your Chrome shortcut > Properties > Target > add ' --remote-debugging-port=9222' to the end.")
                log_callback("================= DETAILED ERROR TRACEBACK ==================")
                log_callback(traceback.format_exc())
                log_callback("=============================================================")
            return

        # --- The rest of the login flow logic remains the same, operating in the new tab ---
        for acc in accounts:
            if check_stop(stop_event):
                break
            # ... (account processing logic is unchanged)
        ...
        try:
            ...
            if log_callback:
                log_callback(f"Note: Could not close the page, you may need to close the tab manually. Error: {e}")
