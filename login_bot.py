...
def start_chrome(log_callback=None):
    """
    Checks for a running Chrome debug session. Does NOT launch Chrome.
    Guides the user on how to launch it manually if not found.
    """
    try:
        res = requests.get(f"http://127.0.0.1:{DEBUG_PORT}/json/version", timeout=2)
        if res.status_code == 200:
            if log_callback:
                log_callback("Successfully connected to existing Chrome debug session.")
            return True
    except requests.exceptions.ConnectionError:
        if log_callback:
            log_callback("=" * 50)
            log_callback("ACTION REQUIRED: Could not connect to Chrome on debug port.")
            log_callback("For automation, Chrome must be in a special debug mode.")
            log_callback("1. Close ALL Chrome windows completely.")
            log_callback("2. Use the 'Create Debug Shortcut' button in the app.")
            log_callback("3. Launch Chrome using the new 'Google Chrome (Debug)' desktop shortcut.")
            log_callback("4. Once Chrome is running, try the automation again.")
            log_callback("=" * 50)
        return False
    except Exception as e:
        if log_callback:
            log_callback(f"An unexpected error occurred while checking for Chrome: {e}")
        return False
